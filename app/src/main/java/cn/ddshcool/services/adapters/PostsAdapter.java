...
//		});
			...
			public void onClick(View v) {
				if(isLiked){
					...
					if(holder.tv_like_bottom.getText().toString().equals("1")){
						...
						holder.tv_like_bottom.setText("" + (Integer.getInteger(
								holder.tv_like_bottom.getText().toString()) + 1));
					}
					holder.tv_like_bottom.setText("");
					delLike(post);
				}else{
					//点赞
	...
	private void setImages(final Post post, FrameLayout imgContainer,
		...
		if(image_urls != null && image_urls.size() > 1) {
			...
			//设置gridView适配器 , 并传入 缩略图
			StatusGridImgsAdapter gvAdapter = new StatusGridImgsAdapter(context, thumbnail_images);
...
//			});
			...
			//读取缩略图
			imageLoader.displayImage(thumbnail_images.get(0), iv_image);
			//转到图片浏览器 ...
